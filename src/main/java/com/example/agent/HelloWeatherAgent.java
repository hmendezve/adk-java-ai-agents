...
//        AdkWebServer.start(ROOT_AGENT);
        ...
        try (Scanner scanner = new Scanner(System.in, UTF_8)) {
            while (true) {
                // Aquí es donde el usuario ingresa su pregunta o comentario, y el agente responde en consecuencia. El bucle continúa hasta que el usuario escribe "quit".
                System.out.print("\nYou > ");
                ...
                //Empaqueta el texto en formato Content requerido por el agente y ejecuta el agente con la entrada del usuario, luego imprime la respuesta del agente en la consola.
                Content userMsg = Content.fromParts(Part.fromText(userInput));
                //Envia el mensaje a los servidores de google de forma asincrona.
                Flowable<Event> events = runner.runAsync(session.userId(), session.id(), userMsg, runConfig);
                System.out.print("\nAgent > ");
                events.blockingForEach(event -> {
                    //Imprime la respuesta del agente en la consola una vez que se recibe la respuesta final del agente.
